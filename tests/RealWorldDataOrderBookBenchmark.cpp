        ...
        for (const ITCH::Message &msg_variant: parsed_itch_for_stock) {
            std::visit([&]<typename T>(T && msg) {
                ...
                if constexpr (std::is_same_v<RealType, ITCH::AddOrderMessage> || std::is_same_v<RealType, ITCH::AddOrderMPIDAttributionMessage>) {
                    ++messages_processed;
                    Order order{msg.order_reference_number, OrderType::Market, msg.shares, msg.price, std::bit_cast<TradeSide>(msg.side)};
                    book.addOrder(order);
                    ...
                    Order new_order{msg.new_order_reference_number, OrderType::Market, msg.new_quantity, msg.new_price, old_order.getSide()};
